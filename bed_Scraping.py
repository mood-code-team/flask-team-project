"""
IKEA 침대(Bed) 통합 상품 수집기 - (다중 카테고리 & 색상 옵션 수집 지원)

[업그레이드 내용]
- 기본 침대뿐만 아니라 소파베드, 데이베드, 쿠션형(패브릭) 침대 카테고리를 모두 순회하며 수집합니다.
- 목표 수량(100개)을 채울 때까지 등록된 여러 카테고리를 차례대로 방문합니다.
"""
...
# ==================================================
# 1. 기본 설정 (★다중 카테고리 URL 적용★)
# ==================================================

CATEGORY_NAME = "BED"

# 이케아의 다양한 침대 카테고리 주소들을 리스트로 묶어줍니다.
CATEGORY_URLS = [
    "https://www.ikea.com/kr/ko/cat/beds-bm003/",             # 기본 침대 프레임
    "https://www.ikea.com/kr/ko/cat/sofa-beds-10663/",        # 소파베드
    "https://www.ikea.com/kr/ko/cat/daybeds-19046/",          # 데이베드
    "https://www.ikea.com/kr/ko/cat/upholstered-beds-24825/"  # 쿠션형(패브릭/가죽) 침대
]

MAX_PRODUCTS = 100
OUTPUT_FOLDER = "output_bed"
IMAGE_FOLDER = os.path.join(OUTPUT_FOLDER, "images")
OUTPUT_EXCEL_FILE = "bed_100_products.xlsx"
OUTPUT_CSV_FILE = "bed_100_products.csv"
CHECKPOINT_FILE = "bed_checkpoint.csv"
...
def collect_product_links(page) -> List[str]:
    """등록된 여러 카테고리 URL을 차례대로 돌며 목표 개수만큼 상품 링크를 모아옵니다."""
    ...
    for category_url in CATEGORY_URLS:
        ...
        try:
            ...
            # 한 카테고리 내에서 더보기 버튼을 누르며 탐색 (최대 30번 시도)
            for load_index in range(30):
                ...
                # 해당 카테고리에서 더 이상 추가 상품이 없으면 다음 카테고리로 넘어감
                if unchanged_count >= 5:
                    print("이 카테고리에서는 더 이상 상품이 로드되지 않아 다음 카테고리로 이동합니다.")
                    break
        except Exception as e:
            print(f"카테고리 탐색 중 오류 발생: {e}")
            continue
...
# ==================================================
# 7. 메인 실행부 (큐 대기열 방식을 활용한 다중 카테고리/색상 수집)
# ==================================================
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            # 설정한 여러 카테고리를 순회하며 초기 링크들을 쓸어 담습니다.
