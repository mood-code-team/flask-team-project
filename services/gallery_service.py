...
def verify_hotspot(
    ...
    """핫스팟 좌표는 유지하고 status만 verified로 변경."""
    hotspot_file = ROOT / "static" / "csv" / "gallery_hotspots.csv"
...
def get_review_scene_navigation(scene_code: str) -> dict:
    """review/failed가 있는 장면 기준으로 이전/다음 검수 장면 반환."""
    hotspot_file = ROOT / "static" / "csv" / "gallery_hotspots.csv"
    ...
    with hotspot_file.open(
        ...
        for row in reader:
            ...
            row_scene_code = (row.get("scene_code") or "").strip()

            if not row_scene_code:
                continue

            if row_scene_code not in review_scene_codes:
                review_scene_codes.append(row_scene_code)

    review_scene_codes.sort()

    if scene_code not in review_scene_codes:
        return {
            "previous": None,
            "next": None,
        }

    current_index = review_scene_codes.index(scene_code)

    previous_scene = None
    next_scene = None

    if current_index > 0:
        previous_scene = review_scene_codes[current_index - 1]

    if current_index < len(review_scene_codes) - 1:
        next_scene = review_scene_codes[current_index + 1]
...
def get_next_review_scene(scene_code: str) -> str | None:
    """현재 장면 다음에 있는 review/failed 검수 장면을 반환."""
    hotspot_file = ROOT / "static" / "csv" / "gallery_hotspots.csv"
    ...
    with hotspot_file.open(
        ...
        for row in reader:
            ...
            code = (row.get("scene_code") or "").strip()

            if code and code not in review_scene_codes:
                review_scene_codes.append(code)

    review_scene_codes.sort()

    # 현재 장면이 아직 검수 목록에 있으면 현재 장면 유지
    if scene_code in review_scene_codes:
        return scene_code

    # 현재 장면이 검수 완료되어 목록에서 빠졌다면
    # 정렬상 현재 코드보다 뒤의 첫 검수 장면으로 이동
    for code in review_scene_codes:
        if code > scene_code:
            return code

    # 뒤쪽에 없다면 남은 검수 장면의 첫 장면
    if review_scene_codes:
        return review_scene_codes[0]

    return None

def get_scene_navigation(scene_code: str) -> dict:
    """전체 갤러리 장면 기준 이전/다음 장면."""
