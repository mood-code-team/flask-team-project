"""
IKEA 소파(Sofa) 상품 수집기 - (검색 결과 기반 & 색상 옵션 수집 지원)

[업그레이드 내용]
- '소파', '암체어', '모듈소파' 검색 결과를 순회하며 수집합니다.
- [핵심] 상품명에 '소파베드'가 포함된 상품은 자동으로 제외(Skip)하고 순수 소파만 수집합니다.
"""
...
# 소파베드를 제외한 순수 소파/암체어를 타겟으로 검색어를 세팅합니다.
CATEGORY_URLS = [
    f"https://www.ikea.com/kr/ko/search/?q={quote('소파')}",
    f"https://www.ikea.com/kr/ko/search/?q={quote('암체어')}",
    f"https://www.ikea.com/kr/ko/search/?q={quote('모듈소파')}"
]

MAX_PRODUCTS = 100
OUTPUT_FOLDER = "output_sofa"
IMAGE_FOLDER = os.path.join(OUTPUT_FOLDER, "images")
OUTPUT_EXCEL_FILE = "sofa_100_products.xlsx"
OUTPUT_CSV_FILE = "sofa_100_products.csv"
CHECKPOINT_FILE = "sofa_checkpoint.csv"
...
# ==================================================
# 7. 메인 실행부 (큐 대기열 방식을 활용한 검색결과/색상 수집 및 필터링)
# ==================================================
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            while queue and len(collected_products) < MAX_PRODUCTS:
                ...
                if product:
                    # ==================================================
                    # ★ [핵심 필터링] 소파베드 상품 자동 제외 로직 ★
                    # ==================================================
                    product_name_no_space = product.get('product_name', '').replace(" ", "")
                    
                    if "소파베드" in product_name_no_space:
                        print(f"  [Skip] 🚨 소파베드 상품으로 확인되어 수집에서 제외합니다: {product.get('product_name')}")
                        continue
                    # ==================================================
                    ...
                    for variant_url in new_color_variants:
                        if variant_url not in visited_urls and variant_url not in queue:
                            ...
                            print(f"  [!] 새로운 색상/옵션 발견 (대기열 추가): {variant_url}")
