...
print(f"🎯 색상/소재 분석 대상 컬럼: [{target_col}]")

# 5. 발코니 용품 특화 색상 및 소재 키워드 정제 추출 함수
def extract_primary_color(text):
    ...
    # 발코니/테라스 용품 특화 키워드 사전
    color_map = {
        '베이지': ['베이지', '라이트베이지', '크림', 'beige', 'cream'],
        '그레이': ['그레이', '다크그레이', '라이트그레이', '회색', '차콜', '타일', '데크타일', '시멘트', 'gray', 'grey'],
        '우드': ['우드', '원목', '목재', '브라운', '갈색', '내추럴', '내츄럴', '데크', 'wood', 'brown', 'natural'],
        '화이트': ['화이트', '아이보리', 'white', 'ivory'],
        '블랙': ['블랙', '검정', 'black'],
        '핑크': ['핑크', '분홍', 'pink'],
        '옐로우': ['옐로우', '노랑', 'yellow'],
        '그린': ['그린', '초록', '잔디', '인조잔디', '식물', '화분', 'green', 'plant'],
